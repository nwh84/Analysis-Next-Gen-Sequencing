...
### 2a. Generate plots for assessing the read distribution across exons/introns
```{}
...
RNA would typically degrade starting at the 5' end. This is because mRNA molecules have a 5' cap and if this is removed then the 5' end is vulnerable to degradation. The 3' end has a poly-a tail which would be more resistant to degradation. 
```{r}
# given code
library(ggplot2); theme_set(theme_bw(base_size = 16)) # for making plots
library(magrittr) # for "pipe"-like coding in R
folder <- "/Users/noelawheeler/Downloads/"
readcounts <- read.table(paste0(folder, "gierlinksi_fc.txt"), header = TRUE)
str(readcounts)
